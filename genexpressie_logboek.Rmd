...
```{r}
barplot(colSums(data[c(control, smoker, vaper)]) / 1e6, las = 2)
```
...
Nu er duidelijk is welke stappen we naar op zoek zijn, kunnen we beginnen met het zoeken naar de DEG's. We gebruiken hiervoor de DESEQ module. Deze module hebben we ook al eerder gebruikt voor de heatmap en mds-plot. Het annotation dataframe wat we voor de heatmap hebben gebruikt kunnen we nu weer gebruiken. Het is wel belangrijk dat de controle groep als eerste voorkomt in het dataframe.
...
# sum(abs(res_control_vs_vaper$log2FoldChange) > 1.5 & res_control_vs_vaper$padj < 0.1, na.rm = T)
```

De genen worden vervolgens geplot in een MA-plot. Wel goed om op te merken is dat er in het originele onderzoek gebruik is gemaakt van een LFC-treshold is gebruikt van 1.5. Deze kan niet worden toegevoegd aan de test. Om de grafiek wat 'duidelijker' te maken zijn er twee lijnen geplot. Een lijn bij -1.5 en 1.5. Alle blauwe stippen die onder/boven deze lijnen liggen kunnen worden beschouwd als DEG. 
...
#deseq.volcano(res = res_control_vs_vaper_shrink, datasetName = "Control - Vaper")
#deseq.volcano(res = res_control_vs_smoker_shrink, datasetName = "Control - Smoker")
```
...
pval_threshold <- 0.1
# In het originele onderzoek is er geen log fold change gebruikt, maar een fold change.
# Dus om terug te gaan, log2(1.5)
lfc_treshold <- log2(1.5)

vaper.degs <- row.names(res_control_vs_vaper_shrink[which(res_control_vs_vaper_lfc$padj <= pval_threshold & res_control_vs_vaper_shrink$log2FoldChange > 0.05 ), ])

smoker.degs <- row.names(res_control_vs_smoker_shrink[which(res_control_vs_smoker_lfc$padj <= pval_threshold & res_control_vs_smoker_shrink$log2FoldChange > 0.05 ), ])
```
