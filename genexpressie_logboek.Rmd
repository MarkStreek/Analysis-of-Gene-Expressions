...
group <- factor(c(rep(1, length(smoker)), 
...
```{r}
counts.fpm <- log2( (data[c(smoker, control, vaper)] / (colSums(data[c(smoker, control, vaper)]) / 1e6)) + 1 )
...
new_data.counts.fpm <- log2( (new_data[c(smoker, control, vaper)] / (colSums(new_data[c(smoker, control, vaper)]) / 1e6)) + 1 )
...
```{r}
dds <- DESeqDataSetFromMatrix(countData = floor(data[9:ncol(data)]),
                              colData = annotation,
                              design= ~ group)
