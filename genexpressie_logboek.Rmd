...
Het valt verder op dat er veel DEG's aan de rechterkant van het figuur bevinden. Dit bevestigd eerder aangetoonde grafieken, waarin werd weergegeven dat de meeste DEG's een expressie waarde hebben die *hoger* ligt dan de controle groep.

### 5.2 Venn Diagram

In de volcano plot kan niet het absolute aantal DEG's worden afgelozen. Om deze reden maken we onderstaand een venn diagram. Dit is een diagram waarin eenvoudig het aantal DEG's kan worden afgelezen. Op deze manier kunnen de verschillen tussen beide groepen ook duidelijker worden. 
...
De venn diagram toont aan dat 619 DEG's worden gedeeld in beide groepen. Voor de rokers groep is een aantal van 1383 DEG's uniek. Totaal bevat de groep dus 619 gedeelde + 1383 unieke = 2002 DEG's

### 5.3 Pad analyse

Nu er duidelijk is welke genen als DEG gekenmerkt worden, kunnen we verder kijken naar de biologische achtergrond. De DEG's worden gegroupeerd op basis van biologisch pad. 
...
DAVID gaat kijken of de opgegeven DEG's voorkomen in processen in het menselijk lichaam. Er wordt ook gegeven hoeveel genen hier betrokken bij zijn. Tot slot wordt er een p-waarde gegeven. Deze waarde geeft aan hoe significant het is, dat dit proces ontregeld is. Let op dat deze waarde niet aangeeft in welke richting dit proces is ontregeld. Deze resultaten kun je downloaden. Onderstaand wordt het bestand ingeladen waar deze resultaten in staan.
...
ggplot(DAVID.df) + geom_bar(aes(x = rownames(DAVID.df),
                     ...
                     coord_flip() + scale_fill_gradient(low = "#eece83", high = "#83A3EE") + labs(title = "P-waardes van de KEGG pathways", subtitle = "-log10 P-waardes uitgezet tegen de pathway titels") + xlab("Titel van de pathways") + ylab("-10log van de P-waardes")
```

Van alle paden die DAVID gevonden heeft bij de opgegeven DEG's, zijn de p-waarden afgezet tegen de omschrijving van het biologische pad. 

Zoals de barplot aantoont is er één proces die er echt uitschiet: Aminoacyl-tRNA biosynthesis. Het laden van de juiste aminzoren aan de juiste tRNA moleculen is dus verstoord. De p-waarde zegt niks over hoe het proces is ontregeld, alleen dat het zo is. We kunnen dus op deze manier niet meteen zeggen of dit proces een hogere regulatie vertoond of juist een lagere.

## Conclusie

Bij de analyse van de rokers DEG's door DAVID, is er een andere uitslag. Bij de analyse van deze genen kwamen andere processen naar boven. Er is dus een verschil zichtbaar tussen vapen en roken. Beide methoden tasten de genexpressie op andere manieren aan. Zoals eerder opgemerkt is er al veel onderzoek gedaan naar roken en is er dus nader gekeken naar de p-waardes van de vaper groep.

Met deze resultaten kunnen we concluderen dat het proces van Aminoacyl-tRNA biosynthesis in leukocyten bij mensen die vapen verstoord is in vergelijking met mensen die niet vapen. Dit is aangetoond aan de hand van de lage p-waarde. De p waarde was siginficant kleiner dan 0.05 (p-waarde: 0.000000000093).
